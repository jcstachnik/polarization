...
my @pfields=qw(dbname orid staname phase hp lp npole
...
if ($hp > 0 && $lp > 0) {
	$filt=sprintf("DEMEAN; BW %0.5f %d %0.5f %d",$hp,$np,$lp,$np);
} else { 
	$filt=sprintf("DEMEAN");
}
...
hp      0.1     	# highpass bounds (> 0)
lp      4.0     	# lowpass bounds 
npole   3       	# Number of BW poles 
